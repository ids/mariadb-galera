...
if [ -z "$NODE_ADDRESS" ]; then
	...
	echo "Resolving Address using eth0"
	...
	DERRIVED_NODE_ADDRESS=$(ip addr | awk "/inet/ && / $NODE_ADDRESS\$/{sub(/\\/.*$/,\"\",\$2); print \$2}" | head -n 1)
	if [[ ! -z "$DERRIVED_NODE_ADDRESS" ]]; then
		echo "Resolved Address using interface mode"
		NODE_ADDRESS=$DERRIVED_NODE_ADDRESS
	fi	
elif ! [[ "$NODE_ADDRESS" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
	# Support grep pattern. E.g. ^10.0.1.*
	echo "Resolving Address using pattern mode"
...
case $START_MODE in
	...
	node)
		...
		while true; do
			...
			if [ $RESOLVE -eq 1 ] && [ $COUNT -lt $(($GCOMM_MINIMUM)) ]; then
				...
				SLEEPS=$((SLEEPS + 1))
				echo "${SLEEPS}: Resolving Nodes: ${COUNT}, GCOMM: ${GCOMM}"
				sleep 2s
			else
				break
			fi

			# After 120 seconds reduce GCOMM_MINIMUM
			if [ $SLEEPS -ge 60 ]; then
				#SLEEPS=0
				#GCOMM_MINIMUM=$((GCOMM_MINIMUM - 1))
				#echo "Reducing GCOMM_MINIMUM to $GCOMM_MINIMUM"
				echo "Bailing on this container because service name DNS does not seem to be resolving."
				exit 1
			fi
		done
		# Pre-boot completed
